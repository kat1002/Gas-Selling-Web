...
public class DeleteDiscountServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (Product p : WebManager.getInstance().productDAO.listProductsByDiscount(did)){
                WebManager.getInstance().productDAO.updateDiscountProductNull(p.getProductId());
            }
            WebManager.getInstance().discountDAO.deleteDiscount(did);
            Discount discount = WebManager.getInstance().discountDAO.getDiscount(did);
            if (discount == null) {
                out.println("<script>");
                out.println("alert('Deleted discount sucessfully ! !');");
                out.println("window.location.href='managediscount';");
                out.println("</script>");
            } else {
                out.println("<script>");
                out.println("alert('Can't delete discount !');");
                out.println("window.location.href='managediscount';");
                out.println("</script>");
            }
