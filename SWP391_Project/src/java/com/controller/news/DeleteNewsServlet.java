 ...
 * @author kirit
 */
@WebServlet(name = "DeleteNewsServlet", urlPatterns = {"/delete-news-post"})
public class DeleteNewsServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int banner =newsDAO.getNews(news_id).getBanner();
            if(banner!=0){
                for (News news : newsDAO.getBanner()) {
                    if (news.getBanner() > banner) {
                        newsDAO.updateNewsBanner(news.getNewsId(), news.getBanner() - 1,news.getModifiedBy());
                    }
                }
            }
